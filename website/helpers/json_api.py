...
class reqArgs:
    ...
    def __call__(self, func):
        def wrapper( *args, **kwargs ):
            ...
            #Are we good?
            if (len(get_missing) + len(post_missing)) + len(sess_missing) > 0:
                return errResponse( request, 'Missing required argument(s): GET%s POST%s SESS%s' % (str(get_missing), str(post_missing), str(sess_missing)))
